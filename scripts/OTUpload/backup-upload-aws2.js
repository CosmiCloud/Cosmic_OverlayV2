...
try {
    ...
    if (output.includes('Backup process complete')) {
        ...
        execSync(`apt install zip`);

        console.log('Compressing upload...');
        execSync(`zip -r ${argv.backupDirectory}/${backupName}/${path.basename(backupTimestamp)}.zip ${argv.backupDirectory}/${backupName}/${path.basename(backupTimestamp)}`);
        ...
        walkSync(`${argv.backupDirectory}/${backupName}`, async (filePath, stat) => {
            ...
            const bucketPath = `${backupName}/${path.basename(backupTimestamp)}.zip`;
        ...
        Promise.all(promises).then(() => {
            ...
            if (fs.existsSync(`${argv.backupDirectory}/${backupName}/${path.basename(backupTimestamp)}.zip`)) {
                execSync(`rm -rf ${argv.backupDirectory}/${backupName}/${path.basename(backupTimestamp)}.zip`);
            }

            if (!argv.TelegramChatID || !argv.TelegramBotToken || !argv.NodeName) {

            }else{
              execSync(`npm install messaging-api-telegram`);

              const { TelegramClient } = require('messaging-api-telegram');
              const client = new TelegramClient({
                accessToken: argv.TelegramBotToken,
              });

              client.sendMessage(argv.TelegramChatID, argv.NodeName+ ' AWS upload process completed!', {
