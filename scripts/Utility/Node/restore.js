...
module.exports ={
    ...
    try{
      ...
      (async () => {
        ...
        if(response.response == 'y' || response.response == 'yes'){
          ...
          console.log('\x1b[32m',"Otnode image has been installed. Copying restore script from image.",'\n');
          var copyscript = "sudo mkdir -p /root/OTRestore && sudo docker cp otnode:/ot-node/current/scripts/restore.sh /root/OTRestore"
          await exec(copyscript);
  ...
  restic_restore: async function restore(){
    try{
      console.log('\x1b[35m','Node Configuration:');
      var nodecfg = 'sudo cat /root/.origintrail_noderc'
      var nodecfg = await exec(nodecfg);
      ...
      if(overlay_config.environment == 'development'){
        var restore =  "sudo /root/OTRestore/restore.sh --environment=development --backupDir=/root/ot-node"
        var image =  'sudo docker create --log-driver json-file --log-opt max-size=50m --name=otnode --hostname='+node_config.network.hostname+' -p 8900:8900 -p 5278:5278 -p 3000:3000 -e LOGS_LEVEL_DEBUG=1 -e SEND_LOGS=1 -v ~/certs/:/ot-node/certs/ -v /root/ot-node/.origintrail_noderc:/ot-node/.origintrail_noderc quay.io/origintrail/otnode-test:feature_blockchain-service'
      }else if(overlay_config.environment == 'testnet'){
        var restore =  "sudo /root/OTRestore/restore.sh --backupDir=/root/ot-node"
        var image =  'sudo docker create --log-driver json-file --log-opt max-size=50m --name=otnode -p 8900:8900 -p 5278:5278 -p 3000:3000 -v /root/ot-node/.origintrail_noderc:/ot-node/.origintrail_noderc origintrail/ot-node:release_testnet'
      }else if(overlay_config.environment == 'mainnet'){
        var restore =  "sudo /root/OTRestore/restore.sh --backupDir=/root/ot-node"
        var image =  'sudo docker create --log-driver json-file --log-opt max-size=50m --name=otnode -p 8900:8900 -p 5278:5278 -p 3000:3000 -v /root/ot-node/.origintrail_noderc:/ot-node/.origintrail_noderc origintrail/ot-node:release_mainnet'
      }
      ...
      (async () => {
        ...
        console.log('\x1b[35mCreating restic password file on /root/ using configured password: '+restic_password);
        ...
        console.log('\x1b[35mRetrieving latest restic snapshot. This may take a while depending onthe amount of data...')
        var latest_snap = 'sudo /root/Cosmic_OverlayV2/restic -r s3:s3.amazonaws.com/'+aws_bucket_name+' snapshots -p /root/restic-password.txt latest'
        var latest_snap = await exec(latest_snap);
        var latest_snap = latest_snap.stdout
        console.log('-->'+latest_snap);

        console.log('\x1b[35mCreating or cleaning /root/restic-backup/')
        var mk_dir = 'sudo mkdir -p /root/restic-backup/ && sudo rm -rf /root/restic-backup/*'
        await exec(mk_dir);

        console.log('\x1b[35mCreating /root/ot-node','\n')
        var mk_dir = 'sudo mkdir -p /root/ot-node/ && sudo rm -rf /root/ot-node/*'
        await exec(mk_dir);

        console.log('\x1b[35mRestoring snapshot to /root/restic-backup/','\n')
        var snap_restore = 'sudo /root/Cosmic_OverlayV2/restic -r s3:s3.amazonaws.com/'+aws_bucket_name+' restore --target /root/restic-backup/ -p /root/restic-password.txt latest'
        await exec(snap_restore,{maxBuffer: 1024 * 900000000});

        console.log('\x1b[35mMoving backup to /root/ot-node/');
        var mv_backup = 'mv -v /root/restic-backup/root/restic-backup/* /root/ot-node/'
        await exec(mv_backup);

        console.log('\x1b[35mMoving .origintrail_noderc to /root/ot-node/','\n');
        var mv_backup = 'mv -v /root/restic-backup/root/restic-backup/.origintrail_noderc /root/ot-node/'
        await exec(mv_backup);
        ...
        console.log('\x1b[35m',"Otnode image has been installed. Copying restore script from image.",'\n');
        var copyscript = "sudo mkdir -p /root/OTRestore && sudo docker cp otnode:/ot-node/current/scripts/restore.sh /root/OTRestore"
        await exec(copyscript);
        ...
        var rm_backup = 'sudo rm -rf /root/restic-backup/*'
        await exec(rm_backup);
