  ...
  newnode: async function install(){
    try{
      ...
      if(overlay_config.environment == 'development'){
        var install = 'sudo docker run --log-driver json-file --log-opt max-size=50m --name=otnode --hostname='+node_config.network.hostname+' -p 8900:8900 -p 5278:5278 -p 3000:3000 -e LOGS_LEVEL_DEBUG=1 -e SEND_LOGS=1 -v ~/certs/:/ot-node/certs/ -v ~/.origintrail_noderc:/ot-node/.origintrail_noderc quay.io/origintrail/otnode-test:feature_blockchain-service'
      }else if(overlay_config.environment == 'testnet'){
        var install = 'sudo docker run -i --log-driver json-file --log-opt max-size=50m --name=otnode -p 8900:8900 -p 5278:5278 -p 3000:3000 -v ~/.origintrail_noderc:/ot-node/.origintrail_noderc origintrail/ot-node:release_testnet'
      }else if(overlay_config.environment == 'mainnet'){
        var install = 'sudo docker run -i --log-driver json-file --log-opt max-size=50m --name=otnode -p 8900:8900 -p 5278:5278 -p 3000:3000 -v ~/.origintrail_noderc:/ot-node/.origintrail_noderc origintrail/ot-node:release_mainnet'
      }
